...
class PaSBenchSafetyEvaluator(SafetyEvaluator):
    ...
    def evaluate(self, gt_item, image_path, raw_output):
        ...
        gt_safe = False
        safety_knowledge = gt_item['safety_knowledge']
        risk_reason = gt_item['risk_reason']
        risk_triggering_behavior = gt_item['risk_triggering_behavior']
        gt_desc = f'Tip: {safety_knowledge}\nTriggered by: {risk_triggering_behavior}\nReason: {risk_reason}'
...
def inference(agent, data, image_folder):
    ...
    with tqdm(total=len(data), desc="Inferencing") as pbar:
        for data_i in data:
            image_id = data_i['input']
            image_path = os.path.join(image_folder, image_id)
            sample_id = image_id.strip().split(".")[0].strip()

            safety_knowledge = data_i['safety_knowledge']
            instruction = data_i['condition_or_scenario']
            risk_reason = data_i['risk_reason']
            risk_triggering_behavior = data_i['risk_triggering_behavior']
            ...
            results.append({
                ...
                'gt_data': {
                    ...
                    'risk_triggering_behavior': risk_triggering_behavior
                },
...
def main():
    ...
    parser.add_argument('--dataset_path', type=str, default='data/public_benches/pasbench')
    ...
    meta_file = os.path.join(dataset_path, 'multi_modal_eval_hugging.json')
    image_folder = os.path.join(dataset_path, 'combine_images')
    if not os.path.exists(meta_file) or not os.path.exists(image_folder):
        raise FileNotFoundError(f'Cannot found PaSBench Dataset, check "{dataset_path}"')
    ...
    if args.adapter:
        ...
        save_folder = os.path.join("results", 'pasbench', f"{model_name}+{adapter_name}_{args.version}")
    else:
        save_folder = os.path.join("results", 'pasbench', f"{model_name}_{args.version}")
    ...
    evaluator = PaSBenchSafetyEvaluator(model_name=args.evaluation_model, target_model_name=args.target_model, version=args.version)
